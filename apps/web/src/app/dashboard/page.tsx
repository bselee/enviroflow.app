...
export default function DashboardPage(): JSX.Element {
  ...
  const enrichedDeviceStateData = useMemo(() => {
    ...
    if (selectedSensor?.ports) {
      for (const port of selectedSensor.ports) {
        ...
        // Ensure line spans full visible range even with sparse history.
        // If first point starts after rangeStart, prepend an anchor point.
        const firstPoint = result[deviceName][0];
        if (firstPoint) {
          const firstTs = new Date(firstPoint.timestamp).getTime();
          if (Number.isFinite(firstTs) && firstTs > rangeStart.getTime()) {
            result[deviceName].unshift({
          ...
          // No historical data: start with current state at range start.
        ...
        const lastTs = lastPoint ? new Date(lastPoint.timestamp).getTime() : NaN;
        const shouldCloseToNow = !Number.isFinite(lastTs) || (now.getTime() - lastTs > 60_000);

        // Add endpoint when state changed, or when history does not reach "now".
        if (!lastPoint || lastPoint.state !== port.isOn || lastPoint.speed !== port.speed || shouldCloseToNow) {
          result[deviceName].push({
            timestamp: nowISO,
            state: port.isOn,
            speed: port.speed,
   ...
   *
    * IMPORTANT: Timeline must use only real captured data.
